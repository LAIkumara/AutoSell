...
export default function UploadAdvertisingFiles(file){
    const promise = new Promise(
        ...
        supabase.storage.from("advertising-images").upload(fileName, file, {
            cacheControl: '3600',
            upsert: false
        }).then(
            ()=> {
            const publicUrl = supabase.storage.from("advertising-images").getPublicUrl(fileName).data.publicUrl;
            resolve(publicUrl);
        }).catch(() => {
